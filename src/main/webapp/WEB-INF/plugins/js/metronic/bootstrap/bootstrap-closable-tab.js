...
var closableTab = {
    ...
    addTab: function (tabItem) { //tabItem = {id,name,url,closable}
        ...
        if (!$('#' + id)[0]) {
            ...
            if (tabItem.closable) {
                li_tab = li_tab + '<i class="icon-close small" tabclose="' + id + '" style="position: absolute;right:4px;top: 4px;"  onclick="closableTab.closeTab(this)"></i></a></li> ';
            } else {
                li_tab = li_tab + '</a></li>';
            }
            var mainHeight = $(document.body).height() - 45;
            var tabpanel = '<div role="tabpanel" class="tab-pane" id="' + container + '" style="width: 100%;">' +
                '<iframe src="' + tabItem.url + '" id="tab_frame_2" frameborder="0" width="100%" height="' + mainHeight + ' marginwidth="0" marginheight="0" scrolling="no" style="overflow-x: hidden; overflow-y: hidden;"  onload="closableTab.frameLoad(this)"></iframe>' +
                '</div>';
